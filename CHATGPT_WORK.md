...
複数エージェントを使用した場合、親エージェントはメインチャットに次の節目を短く表示します。
...
```text
足軽一｜報告：一次資料を確認。運用事例は追加確認が必要。
軍師｜異論：根拠となる資料の公表日が古く、現行運用とは断定できない。
将軍｜判断：結論を保留し、足軽一へ現行資料の再確認を指示。
```

会話らしさを出すために役割と応答関係を表示しますが、存在しない発言や感情を創作しません。エージェントの報告と親エージェントの判断を分け、失敗、意見の相違、未検証事項も表示します。

通常は、出陣、重要な中間報告、追加指示、帰還だけを簡潔に中継します。「実況なし」と指定した場合は編成と完了のみ、「オリジナル風に詳しく」と指定した場合は重要イベントをすべて中継します。細かなツール実行や同じ進捗の繰り返しは表示しません。
